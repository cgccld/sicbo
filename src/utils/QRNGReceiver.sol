...
interface IQRNGReceiver {
  ...
  function getRequestStatus(bytes32 requestId_) external returns (bool fulfilled, uint256[] memory response);
}
...
abstract contract QRNGReceiver is IQRNGReceiver, RrpRequesterV0 {
  ...
  function getRequestStatus(bytes32 requestId_)
    public
    view
  ...
  function fulfillRequest(bytes32 requestId_, bytes calldata data_) external onlyAirnodeRrp onlyRequested(requestId_) {
  ...
  function _additionalHandler(bytes32 requestId_, uint256[] memory response_) internal virtual {}

  function _fulfillRandomWords(bytes32 requestId_, uint256[] memory response_) internal {
  ...
  function _requestRandom() internal {
    ...
    $details[requestId] = RequestDetails({requested: true, fulfilled: false, response: new uint256[](0)});
  ...
  function _isFulfilled(bytes32 requestId_) internal view returns (bool fulfilled) {
    RequestDetails memory req = $details[requestId_];
    fulfilled = req.fulfilled;
  }

  function _isRequested(bytes32 requestId_) internal view returns (bool requested) {
    RequestDetails memory req = $details[requestId_];
