...
contract SicBo is ISicBo, Pausable, ReentrancyGuard, QRNGReceiver, Ownable {
    ...
    function betLow(
        ...
        require(
            amount_ >= sbSettings.minBetAmount,
            "Bet amount must be greater than minBetAmount"
        );
        require(
            ledger[epoch_][_msgSender()].amount == 0,
    ...
    function claim(uint256[] calldata epochs_) external nonReentrant {
        ...
        for (uint256 i; i < epochs_.length; ++i) {
            ...
            require(
                block.timestamp > rounds[epochs_[i]].closeAt,
                "Round has not ended"
            );
            ...
            if (rounds[epochs_[i]].requestedQRNG) {
                require(
                    claimable(epochs_[i], _msgSender()),
                    "Not eligible for claim"
                );
                Round memory round = rounds[epochs_[i]];
                addedReward =
                    (ledger[epochs_[i]][_msgSender()].amount *
                        round.rewardAmount) /
                    round.rewardBaseCalAmount;
            } else {
                require(
                    refundable(epochs_[i], _msgSender()),
    ...
    ) public whenNotPaused onlyOwner onlyFulfilled(requestId_) {
        require(
            genesisStartOnce,
    ...
    function _safeEndRound(
        uint256 epoch_,
        bytes32 requestId_,
        uint256[] memory dicesResult_
    ) internal {
        uint256 bufferSeconds = sbSettings.bufferSeconds;

        require(
            rounds[epoch_].lockAt != 0,
        ...
        require(
            block.timestamp <= rounds[epoch_].closeAt + bufferSeconds,
    ...
    function _safeStartRound(uint256 epoch_) internal {
        ...
        require(
            rounds[epoch_ - 1].closeAt != 0,
            "Can only start round after round n-1 has ended"
        );
        require(
            block.timestamp >= rounds[epoch_ - 1].closeAt,
    ...
    function _preFormatRandomWord(
        uint256[] memory rawRandomWords
    ) internal pure returns (uint256[] memory formattedRandomWords) {
